...
import RouteDetailModal from '../components/RouteDetailModal'; // Modal Import
import {
  MapContainer,
  TileLayer,
  Marker,
...
const OptimizePage = () => {
  ...
  const handleOptimize = async () => {
    ...
    setResults(null); // Sonuçları temizle (Loading ekranı tam görünsün)
    try {
      const response = await runOptimization(scenarioId);
      const solution = response.data.solution;
      
      // Gerçek yolları çek (Her araç rotası için utils fonksiyonunu kullan)
      ...
      setResults(solution); // Veriler hazır olunca göster
    } catch (error) {
      ...
      <div className="flex flex-col md:flex-row justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            Senaryo Tarihi:{' '}
            {new Date(scenario.created_at).toLocaleDateString('tr-TR')}
          </p>
        </div>
        <button
          onClick={handleOptimize}
          disabled={loading}
          className={`mt-4 md:mt-0 px-8 py-3 rounded-xl font-bold text-white shadow-md transition-all transform active:scale-95 ${
            loading
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 flex-grow min-h-[600px]">
          ...
          <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200 max-h-[400px] flex flex-col">
            ...
            <div className="space-y-2 overflow-y-auto pr-2 custom-scrollbar flex-grow">
              {scenario.demands.map((d) => (
                <div
                  key={d.id}
                  className="flex justify-between items-center text-sm p-2 rounded hover:bg-slate-50 border border-transparent hover:border-slate-100"
                >
            ...
            <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200 flex-grow flex flex-col">
              <h2 className="font-bold text-slate-800 mb-4 border-b border-slate-100 pb-2 flex items-center">
                ...
                >
                  <path
                    fillRule="evenodd"
                    d="M12 1.586l-4 4v12.828l4-4V1.586zM3.707 3.293A1 1 0 002 5v11a1 1 0 001.52 1.352l5-1.667.48-.16 2 4V6l-2-4H3.707zM16 3a1 1 0 011 1v11a1 1 0 01-1 1h-3v-2h3V4h-3V3h3z"
                    clipRule="evenodd"
                  />
                </svg>
                Rota Sonuçları
              ...
              <div className="space-y-4 overflow-y-auto pr-2 custom-scrollbar flex-grow">
                {(realRoutes.length > 0 ? realRoutes : results.routes).map((route, idx) => (
                  <div
                    key={idx}
                    onClick={() => openRouteDetail(route, idx)} // Tıklanınca Modalı Aç
                    className="p-3 rounded-lg border-l-4 shadow-sm bg-slate-50 cursor-pointer hover:bg-blue-50 transition transform hover:scale-[1.02]"
                    style={{
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <div>
                        <div className="font-bold text-sm text-slate-900 flex items-center">
                            {route.vehicle.name}
                      ...
                      <div className="text-xs font-mono text-slate-500">
                        {route.load} / {route.vehicle.capacity} kg
                      </div>
                    </div>
        ...
        <div className="lg:col-span-3 bg-white p-2 rounded-xl shadow-sm border border-slate-200 h-[600px] lg:h-auto z-0">
          ...
          >
            ...
            {(realRoutes.length > 0 ? realRoutes : results?.routes || []).map(
              (route, idx) => (
                <Polyline
                  key={idx}
                  ...
                  eventHandlers={{
                      click: () => openRouteDetail(route, idx)
                  }}
                >
                  <Popup>
                    ...
                    <div className="text-xs text-blue-500 mt-1">Detaylar için tıklayın</div>
                  </Popup>
                </Polyline>
              )
            )}
